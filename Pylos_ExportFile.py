...
Provided by Pylos 0.0.03
    ...
    Args:
        ...
        targetFolder_: Input a file path here to the pylos folder on you machine if you are not using the default Github structure that places your pylos github repo in your documents folder.
        ...
        additionalImgs_: A list of file paths to additional images that you want to be shown in Pylos page 
...
def checkTheInputs():
    ...
    if targetFolder_:
        ...
        for file in os.listdir(githubDirectory):
            if 'pylos' in file: pylosFolder = githubDirectory + file + "\\"
        if pylosFolder == None: pylosFolder = "C:\\Users\\" + username + "\\Documents\\GitHub\\pylos\\"
        if not os.path.isdir(pylosFolder):
            checkData1 = False
            warning = "The component cannot automatically find the pylos repo on your machine. \n Plug in the correct file path of the pylos repo to the targetFolder_ input on this component."
            print warning
            ghenv.Component.AddRuntimeMessage(w, warning)
        else:
    ...
    if repoTargetFolder != None:
        ...
        if os.path.isfile(configFile):
            ...
            checkData2 = False
            warning = "Could not find the config file in your Pylos fork on your system. \n Make sure that you have cloned pylos to your system. \n If you have cloned pylos, plug in the correct file path of the pylos repo to the targetFolder_ input on this component."
            print warning
            ghenv.Component.AddRuntimeMessage(w, warning)
...
def writeReadMe(fileName, fileDescription, repoTargetFolder, changeLog, gitUserName, fileTags, forkName):
    ...
    #Write in a link to the user's page and to the pylos page.
    readMeFile.write('This file has been submitted by [' + gitUserName + '](https://github.com/' + gitUserName + ')')
    readMeFile.write('\n')
    readMeFile.write('\n')
    readMeFile.write('[Check out this example on Pylos!](https://viz.zgf.com/pylos/viewer.html?owner=' + gitUserName + '&fork=' + forkName + '&id=' + fileName + ')')
    readMeFile.write('\n')
    ...
    readMeFile.write("![Screenshot](https://raw.githubusercontent.com/" + gitUserName + "/pylos/master/" + fileName + "/thumbnail.png)")
    readMeFile.write('\n')
